...
2. `af-plan` or `af-research` creates the shared initiative folder before the first artifact is written, so research, plans, and later status artifacts all share the same numbered location.
3. `af-plan` delegates bounded `af-research` work, which writes a research artifact under `engineering-context/active/0002_header-rollout_GATE-123/research/`.
4. The research identifies the owning repo for the header contract, the downstream consumers, the rollout constraints, and any version-sensitive framework behavior that needs official docs.
