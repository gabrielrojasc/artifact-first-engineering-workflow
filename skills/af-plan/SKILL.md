...
**Questioning**: Interview the human one question at a time, resolving plan-changing unknowns and dependencies between decisions one by one until you reach shared understanding. Ask a single question, provide your recommended answer, and wait for the response before asking the next. If a question can be answered by exploring the codebase or existing research artifacts, do that instead of asking. Do not batch questions unless the human explicitly asks for a batch.

Alignment is not complete while a plan-changing unknown remains unresolved. Resolve each plan-changing unknown through repository or artifact evidence first; when evidence cannot decide it, ask exactly one narrow question with your recommended answer and wait. If the human explicitly defers the unknown or the task scope explicitly excludes it, state the deferral, owner, and risk before proceeding.
